...
- [`vscode-ripgrep/`](./vscode-ripgrep/) — Replaces [`@vscode/ripgrep`](https://www.npmjs.com/package/@vscode/ripgrep) (downloads platform-specific ripgrep binary at postinstall) with [`ripgrep`](https://github.com/pithings/ripgrep-node), a WebAssembly-based ripgrep that works across all platforms without native binaries. **Legacy:** since VS Code 4.127 the bundle no longer references `@vscode/ripgrep` directly (see `vscode-ripgrep-universal/` below); this shim is kept only for extensions that still `require("@vscode/ripgrep")`.
- [`vscode-ripgrep-universal/`](./vscode-ripgrep-universal/) — Replaces [`@vscode/ripgrep-universal`](https://www.npmjs.com/package/@vscode/ripgrep-universal) (bundles ripgrep binaries for _all_ platforms under `bin/<os>-<arch>/rg`) with the same WebAssembly-based [`ripgrep`](https://github.com/pithings/ripgrep-node). As of VS Code 4.127 the bundle loads this package instead of `@vscode/ripgrep` for text search — via ESM `await import("@vscode/ripgrep-universal")` reading `.rgPath` in `server-main.js`, `agentHostMain.js`, and `extensionHostProcess.js` — and internally aliases `vscode-ripgrep`/`@vscode/ripgrep` to it for extensions. Without this shim, search silently breaks (the package isn't installed and `@vscode/ripgrep` is no longer referenced). The opt-in agent-host sandbox (`chat.agentHost.*`) also builds a hardcoded `node_modules/@vscode/ripgrep-universal/bin/<os>-<arch>/rg` path; that feature depends on other unshimmed deps (`@vscode/sandbox-runtime`, mxc runtime) and is inert in coderaft, so the per-platform `bin/` layout is intentionally not provided.
...
- [`vscode-sqlite3/`](./vscode-sqlite3/) — Replaces [`@vscode/sqlite3`](https://github.com/microsoft/vscode-node-sqlite3) (native SQLite binding, bundled C source + `node-gyp` build, ~3 MB unpacked) with a thin async wrapper around Node.js' built-in synchronous `node:sqlite` (stable in Node 24+). Loaded via dynamic `import("@vscode/sqlite3")` from `vs/platform/agentHost/node/agentHostMain.js`; only the `Database`/`exec`/`run`/`get`/`all`/`close` surface is needed.
- [`ssh2/`](./ssh2/) — Stub for [`ssh2`](https://github.com/mscdex/ssh2). Listed as a direct dep by upstream code-server but not actually loaded by the bundled VS Code server, CLI, bootstrap-fork, or any extension — the only references in the build output are embedded copies of code-server's `package.json` `overrides` block. Eliminates ssh2 itself plus its native transitive deps (`cpu-features`, `nan`, `asn1`, `bcrypt-pbkdf`, `tweetnacl`, `buildcheck`).
...
| `@vscode/sqlite3`              | `shims/vscode-sqlite3/`          |
...
| `@vscode/ripgrep-universal`    | `shims/vscode-ripgrep-universal/`|
